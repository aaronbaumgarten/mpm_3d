...
with open(particle_filename, 'w') as f:
    ...
    #    mpm_points.append(mpm_point)
        f.write("%g %g %g %g %g %g %g %g %g %g\n" % (0, pID, block_properties['rho']*grid.material_point_volume, grid.material_point_volume, p.x, p.y, p.z, 1, 0, 1))
        pID += 1
    pID = 0
    for p in block2_point_array:
        f.write("%g %g %g %g %g %g %g %g %g %g\n" % (1, pID, block2_properties['rho']*grid.material_point_volume, grid.material_point_volume, p.x, p.y, p.z, -1, 0, 1))
        pID += 1

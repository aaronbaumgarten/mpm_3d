...
block_depth = 1.0
hx = Lx/Nx
block_primitive = Primitives3d.Box(0.0, block_width,
                                 0.6, 0.6+block_height,
                                 0, 1.0,
                                 )
...
with open(particle_filename, 'w') as f:
    ...
    for p in block_point_array:
        true_volume = grid.material_point_volume*2*math.pi*p.x
    #    mpm_point = {
    #                    'body': 1,
    ...
    #    mpm_points.append(mpm_point)
        f.write("%g %g %g %g %g %g %i\n" % (block_properties['rho']*true_volume, grid.material_point_volume, p.x, p.y, 0, -3, 1))

...
block_depth = 1.0
hx = Lx/Nx
block_primitive = Primitives3d.Box(0, 2.0,
                                 0, 0.2,
                                 0, 1.0,
                                 )
...
with open(particle_filename, 'w') as f:
    ...
    #    mpm_points.append(mpm_point)
        tmp_rho = 0
        if p.x < 1.0:
            tmp_rho = block_properties['rho']*(0.1 + 0.5*p.x)
        else:
            tmp_rho = block_properties['rho']*(1.1 - 0.5*p.x)
        
        f.write("%g %g %g %g %g %g %i\n" % (tmp_rho*grid.material_point_volume, grid.material_point_volume, p.x, p.y, 0, 0, 1))

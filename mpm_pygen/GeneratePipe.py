...
block_primitive = Primitives3d.Box(hx/2.0,Lx-hx/2.0,
                                 0, block_depth,
                                 0, block_height,
                                 )
...
with open(particle_filename, 'w') as f:
    ...
    for p in block_point_array:
        pointDensity = block_properties['rho']*grid.material_point_volume
        if p.y < grain_depth:
            pointDensity *= 0.4
        f.write("%g %g %g %g %g %g %g %g %g %g\n" % (1, pID, pointDensity, grid.material_point_volume, p.x, p.y, p.z, 0, 0, 0))
        pID += 1

...
void boundaryGenerateRules(Job* job, Body* body){
    ...
    bcNodalForce = job->jobVectorArray<double>(body->nodes.f.rows());
    return;
}

/*----------------------------------------------------------------------------*/

void boundaryApplyRules(Job* job, Body* body){Eigen::VectorXd delta_momentum = job->jobVector<double>();
    double f_normal;
    ...
    //handle 2D case
    if (job->DIM == 2){
        tmpT = job->jobTensorArray<double>(body->points.T.rows());
        tmpT.col(0) = body->points.T.col(0);
        tmpT.col(1) = body->points.T.col(1);
        tmpT.col(2) = body->points.T.col(3);
        tmpT.col(3) = body->points.T.col(4);
        body->bodyCalcNodalDivergence(job, bcNodalForce, tmpT, Body::ADD);
    } else {

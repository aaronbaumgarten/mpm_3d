...
// cartesian_periodic_drivenstep.cpp
//
...
double velocity1, velocity2, switch_time; //driven velocity
...
void boundaryInit(Job* job, Body* body){
    ...
    if (body->boundary.fp64_props.size() < 3) {
        ...
        fprintf(stderr,
                "%s:%s: Need at least 3 property defined (velocity1, velocity2, switch_time).\n",
                __FILE__, __func__);
        exit(0);
    } else {
        //store stop_time
        velocity1 = body->boundary.fp64_props[0];
        velocity2 = body->boundary.fp64_props[1];
        switch_time = body->boundary.fp64_props[2];

        //print grid properties
        std::cout << "Boundary properties (velocity1 = " << velocity1 << ", velocity2 = " << velocity2 << ", switch_time = " << switch_time << ")." << std::endl;
    }
...
int boundaryLoadState(Job* job, Body* body, Serializer* serializer, std::string fullpath){
    ...
    if (fin.is_open()) {
        ...
        std::getline(fin,line); //velocity1
        velocity1 = std::stod(line);
        std::getline(fin,line); //velocity2
        velocity2 = std::stod(line);
        std::getline(fin,line); //switch
        switch_time = std::stod(line);
    ...
    for (size_t i=0;i<body->nodes.x_t.rows();i++){
        for (size_t pos=0;pos<body->nodes.x_t.cols();pos++){
            if (bcNodalMask(i,pos) == 2){
                if (job->t >= switch_time) {
                    driving_force(i) = (body->nodes.m(i) * velocity2 - body->nodes.mx_t(i, pos)) / job->dt;
                } else {
                    driving_force(i) = (body->nodes.m(i) * velocity1 - body->nodes.mx_t(i, pos)) / job->dt;
                }
            }
        }
    }

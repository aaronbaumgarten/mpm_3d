...
void generate_dirichlet_bcs(job_t *job)
{
    ...
    // set ceiling value
    for (size_t nx = 0; nx < job->Nx; nx++) {
        job->u_dirichlet[NODAL_DOF * (nx + job->Nx*(job->Ny-1)) + YDOF_IDX] = 0;
        job->u_dirichlet_mask[NODAL_DOF * (nx + job->Nx*(job->Ny-1)) + YDOF_IDX] = 1;

        job->u_dirichlet[NODAL_DOF * (nx + job->Nx*job->Ny + job->Nx*(job->Ny-1)) + YDOF_IDX] = 0;
        job->u_dirichlet_mask[NODAL_DOF * (nx + job->Nx*job->Ny + job->Nx*(job->Ny-1)) + YDOF_IDX] = 1;


        job->u_dirichlet[NODAL_DOF * (nx + job->Nx*(job->Ny-1)) + XDOF_IDX] = 1;
        job->u_dirichlet_mask[NODAL_DOF * (nx + job->Nx*(job->Ny-1)) + XDOF_IDX] = 1;

        job->u_dirichlet[NODAL_DOF * (nx + job->Nx*job->Ny + job->Nx*(job->Ny-1)) + XDOF_IDX] = 1;
        job->u_dirichlet_mask[NODAL_DOF * (nx + job->Nx*job->Ny + job->Nx*(job->Ny-1)) + XDOF_IDX] = 1;
    }

    // set wall value (0 + Nx*n),(Nx-1 + Nx*n)
    for (size_t ny = 0; ny < job->Ny*job->Nz; ny++) {
        job->u_dirichlet[NODAL_DOF * (job->Nx * ny) + YDOF_IDX] = 0;
        job->u_dirichlet_mask[NODAL_DOF * (job->Nx * ny) + YDOF_IDX] = 1;

        job->u_dirichlet[NODAL_DOF * (job->Nx * (ny+1) - 1) + YDOF_IDX] = 0;
        job->u_dirichlet_mask[NODAL_DOF * (job->Nx * (ny+1) - 1) + YDOF_IDX] = 1;
        ...
        for (size_t j = 0; j < NODAL_DOF; j++) {
            ...
            if (job->u_dirichlet_mask[m] != 0) {
                ...
                if (job->u_dirichlet[m] == 0) {
                    ...
                    for (size_t b = 0; b < job->num_bodies; b++) {
                        if (j == XDOF_IDX) {
                            job->bodies[b].nodes.contact_mx_t[i] = 1.0 * job->bodies[b].nodes.m[i];
                        } else if (j == YDOF_IDX) {
                            job->bodies[b].nodes.contact_my_t[i] = 0;
                        } else if (j == ZDOF_IDX) {
                            job->bodies[b].nodes.contact_mz_t[i] = 0;
        ...
        for (size_t j = 0; j < NODAL_DOF; j++) {
            ...
            if (job->u_dirichlet_mask[m] != 0) {
                ...
                if (job->u_dirichlet[m] == 0) {
                    ...
                    for (size_t b = 0; b < job->num_bodies; b++) {
                        if (j == XDOF_IDX) {
                            job->bodies[b].nodes.contact_fx[i] = (-job->bodies[b].nodes.mx_t[i] + 1.0 * job->bodies[b].nodes.m[i]) / job->dt;;
                        } else if (j == YDOF_IDX) {
                            job->bodies[b].nodes.contact_fy[i] = 0;
                        } else if (j == ZDOF_IDX) {
                            job->bodies[b].nodes.contact_fz[i] = 0;

...
//helper functions
int FVMGmsh3D::whichElement(Job* job, KinematicVector xIN){
    std::cout << "ERROR: whichElement not implemented for FVMGmsh3D. Fatal. Exiting." << std::endl;
    exit(0);
} //return which element this position is within

std::vector<double> FVMGmsh3D::getBoundingBox(){
    std::cout << "ERROR: getBoundingBox not implemented for FVMGmsh3D. Fatal. Exiting." << std::endl;
    exit(0);
}  //return min/max of each coordinate


void FVMGmsh3D::init(Job* job, FiniteVolumeDriver* driver){
    //assign grid dimension
    GRID_DIM = 3;
    //check that MPM grid is also 2D
    if (job->grid->GRID_DIM != 3){
        ...
        for (int i=0; i<tmp_bc_info.size(); i++){
            ...
            switch (tmp_bc_info[i].tag){
                ...
                case PERIODIC:
                    ...
                    std::cerr << "ERROR: FVMGmsh3D does not allow PERIODIC boundaries! Exiting." << std::endl;
                    exit(0);
                    break;
    ...
    if (job->JOB_TYPE != 3){
        std::cerr << "WARNING: Cannot use FVMGmsh3D with JOB_TYPE of " << job->JOB_TYPE << "!" << std::endl;
    }

    //temporary vector listing faces on boundary
    std::vector<std::array<int,4>> bounding_faces; //id, node1, node2, node3
                ...
                case 2:
                    ...
                    if (floor(msh_version) == 2) {
                        ...
                        while (std::getline(fin, line)) {
                            ...
                            } else if (std::stoi(lvec[1]) == 2) {
                                //face
                                len = std::stoi(lvec[2]); //num tags
                                std::array<int,4> tmp_array = std::array<int,4>();
                                tmp_array[0] = std::stoi(lvec[3]);
                                tmp_array[1] = std::stoi(lvec[3 + len]) - 1;
                                tmp_array[2] = std::stoi(lvec[4 + len]) - 1;
                                tmp_array[3] = std::stoi(lvec[5 + len]) - 1;
                        ...
                        while (std::getline(fin, line)){
                            ...
                            block_length = std::stoi(lvec[3]);
                            //check that entity is tetrahedron
                            if (block_type == 4) {
    ...
    for (int e=0; e<nodeIDs.rows(); e++){
        ...
        v_e(e) = std::abs((a.cross(b)).dot(c))/6.0;

        //element centroid
        x_e[e] = (x_n(nodeIDs(e,0)) + x_n(nodeIDs(e,1)) + x_n(nodeIDs(e,2)) + x_n(nodeIDs(e,3)))/4.0;
    }
    ...
    std::vector<std::vector<std::array<int,3>>> tmp_node_to_face_map = std::vector<std::vector<std::array<int,3>>>(node_count);
    //[node id]: [other node, other node, face_id]
    int f_tmp=0;
    int n0, n1, n2, n3;
    bool face_defined;
    for (int e=0; e<element_count; e++){
        ...
        for (int i=0; i<tmp_node_to_face_map[n0].size(); i++){
            if ((n1 == tmp_node_to_face_map[n0][i][0] || n1 == tmp_node_to_face_map[n0][i][1]) &&
                (n2 == tmp_node_to_face_map[n0][i][0] || n2 == tmp_node_to_face_map[n0][i][1])){
                //face is defined
                face_defined = true;
                f_tmp = tmp_node_to_face_map[n0][i][2]; //store face id
                //exit loop
                break;
            }
        }
        ...
        if (!face_defined){
            ...
            std::array<int,2> tmp_e_map = {e, -1};
            face_elements.push_back(tmp_e_map);
        } else {
            //add element to face map
        ...
        for (int i=0; i<tmp_node_to_face_map[n0].size(); i++){
            if ((n1 == tmp_node_to_face_map[n0][i][0] || n1 == tmp_node_to_face_map[n0][i][1]) &&
                (n3 == tmp_node_to_face_map[n0][i][0] || n3 == tmp_node_to_face_map[n0][i][1])){
                //face is defined
                face_defined = true;
                f_tmp = tmp_node_to_face_map[n0][i][2]; //store face id
                //exit loop
        ...
        //if face is not defined, then define it
        if (!face_defined){
            std::array<int,3> tmp_array = {n0, n1, n3};
            face_nodes.push_back(tmp_array);                //add nodes to define face
            f_tmp = face_nodes.size() - 1;
            tmp_array = {n0, n1, f_tmp};
            tmp_node_to_face_map[n3].push_back(tmp_array);  //add to temporary container so we don't duplicate faces
            tmp_array = {n1, n3, f_tmp};
            tmp_node_to_face_map[n0].push_back(tmp_array);  //add to temporary container so we don't duplicate faces
            tmp_array = {n3, n0, f_tmp};
            tmp_node_to_face_map[n1].push_back(tmp_array);  //add to temporary container so we don't duplicate faces
        ...
        face_defined = false;
        for (int i=0; i<tmp_node_to_face_map[n0].size(); i++){
            if ((n2 == tmp_node_to_face_map[n0][i][0] || n2 == tmp_node_to_face_map[n0][i][1]) &&
                (n3 == tmp_node_to_face_map[n0][i][0] || n3 == tmp_node_to_face_map[n0][i][1])){
                //face is defined
                face_defined = true;
                f_tmp = tmp_node_to_face_map[n0][i][2]; //store face id
        ...
        //if face is not defined, then define it
        if (!face_defined){
            std::array<int,3> tmp_array = {n0, n2, n3};
            face_nodes.push_back(tmp_array);                //add nodes to define face
            f_tmp = face_nodes.size() - 1;
            tmp_array = {n0, n2, f_tmp};
            tmp_node_to_face_map[n3].push_back(tmp_array);  //add to temporary container so we don't duplicate faces
            tmp_array = {n2, n3, f_tmp};
            tmp_node_to_face_map[n0].push_back(tmp_array);  //add to temporary container so we don't duplicate faces
            tmp_array = {n3, n0, f_tmp};
            tmp_node_to_face_map[n2].push_back(tmp_array);  //add to temporary container so we don't duplicate faces
        ...
        for (int i=0; i<tmp_node_to_face_map[n1].size(); i++){
            if ((n2 == tmp_node_to_face_map[n1][i][0] || n2 == tmp_node_to_face_map[n1][i][1]) &&
                (n3 == tmp_node_to_face_map[n1][i][0] || n3 == tmp_node_to_face_map[n1][i][1])){
                //face is defined
                face_defined = true;
                f_tmp = tmp_node_to_face_map[n1][i][2]; //store face id
                //exit loop
                break;
            }
        }
        //if face is not defined, then define it
        if (!face_defined){
            std::array<int,3> tmp_array = {n1, n2, n3};
            face_nodes.push_back(tmp_array);                //add nodes to define face
            f_tmp = face_nodes.size() - 1;
            tmp_array = {n1, n2, f_tmp};
            tmp_node_to_face_map[n3].push_back(tmp_array);  //add to temporary container so we don't duplicate faces
    ...
    //consistency check
    for (int e=0; e<element_count; e++){
        if (element_faces[e].size() != 4){
            std::cout << "ERROR! Element " << e << " has incorrect number of faces: " << element_faces[e].size() << " != 4." << std::endl;
        }
    }
    ...
    for (int f=0; f<face_count; f++){
        ...
        //face area = 0.5*||axb||
        tmpVec = KinematicVector((x_n[n1] - x_n[n0]).cross((x_n[n2] - x_n[n1])),job->JOB_TYPE);
        face_areas(f) = 0.5*tmpVec.norm();

        //face centroid
        x_f[f] = (x_n[n0] + x_n[n1] + x_n[n2])/3.0;

        //face normal
        face_normals[f] = tmpVec;
    ...
    for (int i=0; i<bounding_faces.size(); i++){
        ...
        n2 = bounding_faces[i][3];
        //check faces associated with n0 for n1,n2 from node to face map create earlier
        for (int ii=0; ii<tmp_node_to_face_map[n0].size(); ii++){
            if ((n1 == tmp_node_to_face_map[n0][ii][0] || n1 == tmp_node_to_face_map[n0][ii][1]) &&
                (n2 == tmp_node_to_face_map[n0][ii][0] || n2 == tmp_node_to_face_map[n0][ii][1])){
                //found face
                f_tmp = tmp_node_to_face_map[n0][ii][2];
                break;
            }
        }
        if (which_boundary >= tmp_bc_info.size()){
            ...
            if (driver->TYPE == FiniteVolumeDriver::ISOTHERMAL) {
                ...
                std::cerr << "ERROR: FVMGmsh3D is not implemented for FiniteVolumeDriver TYPE " << driver->TYPE << "! Exiting.";
                exit(0);
            }
        } else {
    ...
    if (driver->ORDER == 1 || USE_REDUCED_QUADRATURE){
        ...
        std::cerr << "ERROR: FVMGmsh3D not defined for simulation ORDER > 2." << std::endl;
        qpe = 4;
        qpf = 3;
    }
    ...
    for (int e=0; e<element_count; e++){
        if (qpe == 1){
            ...
            x_q[e*qpe + 0] = x_n[nodeIDs(e,0)]
                             + 0.5854101966249685*x_n[nodeIDs(e,1)]
                             + 0.1381966011250105*x_n[nodeIDs(e,2)]
                             + 0.1381966011250105*x_n[nodeIDs(e,3)];
            w_q(e*qpe + 0) = 0.25*v_e[e];

            x_q[e*qpe + 1] = x_n[nodeIDs(e,0)]
                             + 0.1381966011250105*x_n[nodeIDs(e,1)]
                             + 0.1381966011250105*x_n[nodeIDs(e,2)]
                             + 0.1381966011250105*x_n[nodeIDs(e,3)];
            w_q(e*qpe + 1) = 0.25*v_e[e];

            x_q[e*qpe + 2] = x_n[nodeIDs(e,0)]
                             + 0.1381966011250105*x_n[nodeIDs(e,1)]
                             + 0.1381966011250105*x_n[nodeIDs(e,2)]
                             + 0.5854101966249685*x_n[nodeIDs(e,3)];
            w_q(e*qpe + 2) = 0.25*v_e[e];

            x_q[e*qpe + 3] = x_n[nodeIDs(e,0)]
                             + 0.1381966011250105*x_n[nodeIDs(e,1)]
                             + 0.5854101966249685*x_n[nodeIDs(e,2)]
                             + 0.1381966011250105*x_n[nodeIDs(e,3)];
            w_q(e*qpe + 3) = 0.25*v_e[e];
        }
    }
    ...
    for (int f=0; f<face_count; f++){
        ...
        if (qpf == 1){
            ...
            x_q[int_quad_count + f*qpf + 0] = 0.5*(x_f[f] + x_n[face_nodes[f][0]]);
            w_q(int_quad_count + f*qpf + 0) = getFaceArea(f)/qpf;
            x_q[int_quad_count + f*qpf + 1] = 0.5*(x_f[f] + x_n[face_nodes[f][1]]);
            w_q(int_quad_count + f*qpf + 1) = getFaceArea(f)/qpf;
            x_q[int_quad_count + f*qpf + 2] = 0.5*(x_f[f] + x_n[face_nodes[f][2]]);
            w_q(int_quad_count + f*qpf + 2) = getFaceArea(f)/qpf;
        }
    }
...
void FVMGmsh3D::writeHeader(std::ofstream& file, int SPEC){
    ...
    for (int i=0;i<node_count;i++){
        ...
        file << x_n(i,0) << " " << x_n(i,1) << " " << x_n(i,2) << "\n";
    }
    file << "CELLS " << element_count << " " << 5*element_count << "\n";
    for (int e=0;e<element_count;e++){
        file << "4 " << nodeIDs(e,0) << " " << nodeIDs(e,1) << " " << nodeIDs(e,2) << " " << nodeIDs(e, 3) << "\n";
    }
...
void FVMGmsh3D::cSOMF(Job *job, FiniteVolumeDriver *driver, int k_begin, int k_end) {
    if (driver->ORDER == 1){
        ...
        if (driver->ORDER > 2) {
            std::cout << "ERROR: FVMGmsh3D does not currently implement higher ORDER momentum reconstruction."
                      << std::endl;
        }
...
void FVMGmsh3D::cSODF(Job *job, FiniteVolumeDriver *driver, int k_begin, int k_end) {
    if (driver->ORDER == 1){
        for (int e=k_begin; e<=k_end; e++) {
...
void FVMGmsh3D::cSOEF(Job *job, FiniteVolumeDriver *driver, int k_begin, int k_end) {
    if (driver->ORDER == 1){
        for (int e=k_begin; e<=k_end; e++) {
            driver->fluid_body->rhoE_x[e].setZero(); //let density be constant within an element
        }
    } if (driver->ORDER >= 2){
        if (driver->ORDER > 2) {
            std::cout << "ERROR: FVMGmsh3D does not currently implement higher ORDER momentum reconstruction."
...
void FVMGmsh3D::cSOPF(Job *job, FiniteVolumeDriver *driver, int k_begin, int k_end) {
    ...
    if (USE_LOCAL_POROSITY_CORRECTION) {
        ...
        for (int e = k_begin; e <= k_end; e++) {
            ...
            if (driver->ORDER >= 2) {
                if (driver->ORDER > 2) {
                    std::cout << "ERROR: FVMGmsh3D does not currently implement higher ORDER porosity reconstruction." << std::endl;
                }
...
void FVMGmsh3D::constructSubsetOfMomentumField(Job* job,
...
void FVMGmsh3D::constructSubsetOfDensityField(Job* job,
                                                 FiniteVolumeDriver* driver,
                                                 FVMGmsh3D* grid,
                                                 int k_begin, int k_end,
                                                 volatile bool &done){
...
void FVMGmsh3D::constructSubsetOfEnergyField(Job* job,
                                                FiniteVolumeDriver* driver,
                                                FVMGmsh3D* grid,
                                                int k_begin, int k_end,
                                                volatile bool &done){
...
void FVMGmsh3D::constructSubsetOfPorosityField(Job* job,
                                                  FiniteVolumeDriver* driver,
                                                  FVMGmsh3D* grid,
                                                  int k_begin, int k_end,
                                                  volatile bool &done){

...
void materialCalculateStress(Job* job, Body* body, int SPEC){
    ...
    for (size_t i=0;i<body->points.x.rows();i++){
        ...
        for (size_t pos=0;pos<body->points.x.cols();pos++) {
            u = (((double)std::rand())/(RAND_MAX));
            if (u < 0.5) {
                dx = 0.1 * (std::sqrt(2.0 * u) - 1.0);
            } else if (u > 0.5) {
                dx = 0.1 * (1.0 - std::sqrt(2.0 * (1.0 - u)));
            } else {
                dx = 0;
            }
            body->points.x(i,pos) += dx*body->points.extent(i);
        }
    }

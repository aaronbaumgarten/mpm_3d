...
double CartesianQuadratic::s(double x, double h){
    if (x < -1.5*h){
        return 0;
    } else if (x < -0.5*h){
        return 0.5*x*x/(h*h) + 1.5*x/h + 1.125;
    } else if (x < 0.5*h){
        return -x*x/(h*h) + 0.75;
    } else if (x < 1.5*h){
        return 0.5*x*x/(h*h) - 1.5*x/h + 1.125;
...
double CartesianQuadratic::g(double x, double h){
    if (x < -1.5*h){
        return 0;
    } else if (x < -0.5*h){
        return x/(h*h) + 1.5/h;
...
void CartesianQuadratic::init(Job* job){
...
/*----------------------------------------------------------------------------*/
//hidden initializer
void CartesianQuadratic::hiddenInit(Job* job){//called from loading or initializing functions
...
/*----------------------------------------------------------------------------*/
//
void CartesianQuadratic::writeFrame(Job* job, Serializer* serializer){
...
/*----------------------------------------------------------------------------*/
//
std::string CartesianQuadratic::saveState(Job* job, Serializer* serializer, std::string filepath){
    return "err";
}
...
/*----------------------------------------------------------------------------*/
//
int CartesianQuadratic::loadState(Job* job, Serializer* serializer, std::string fullpath){
    return 0;
}
...
/*----------------------------------------------------------------------------*/
//
int CartesianQuadratic::whichElement(Job* job, KinematicVector& xIN){
    return CartesianLinear::cartesianWhichElement(job, xIN, Lx, hx, Nx, GRID_DIM);
}

/*----------------------------------------------------------------------------*/
//
bool CartesianQuadratic::inDomain(Job* job, KinematicVector& xIN){
...
/*----------------------------------------------------------------------------*/
//
KinematicVector CartesianQuadratic::nodeIDToPosition(Job* job, int idIN){
...
/*----------------------------------------------------------------------------*/
//
void CartesianQuadratic::evaluateBasisFnValue(Job* job, KinematicVector& xIN, std::vector<int>& nID, std::vector<double>& nVAL){
...
/*----------------------------------------------------------------------------*/
//
void CartesianQuadratic::evaluateBasisFnGradient(Job* job, KinematicVector& xIN, std::vector<int>& nID, KinematicVectorArray& nGRAD){
...
/*----------------------------------------------------------------------------*/
//
double CartesianQuadratic::nodeVolume(Job* job, int idIN){
    return v_n(idIN);
}
...
/*----------------------------------------------------------------------------*/
//
double CartesianQuadratic::elementVolume(Job* job, int idIN){
    return v_e;
}
...
/*----------------------------------------------------------------------------*/
//
int CartesianQuadratic::nodeTag(Job* job, int idIN){
    return -1;
}

/*----------------------------------------------------------------------------*/
//
double CartesianQuadratic::nodeSurfaceArea(Job *job, int idIN) {
    return s_n(idIN);
}

/*----------------------------------------------------------------------------*/
// return edge list
std::vector<int> CartesianQuadratic::getEdgeList(Job* job){
...
/*----------------------------------------------------------------------------*/
//
void CartesianQuadratic::writeHeader(Job *job, Body *body, Serializer *serializer, std::ofstream &nfile, int SPEC) {

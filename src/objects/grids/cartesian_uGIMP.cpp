...
double CartesianUGIMP::s(double x, double h, double l){
    if (x < -h-l){
        return 0;
    } else if (x < -h+l){
        return (h + l + x)*(h + l + x)/(4.0 * l * h);
    } else if (x < -l) {
        return 1.0 + x/h;
    } else if (x < l) {
        return 1.0 - (x*x + l*l)/(2.0 * l * h);
    } else if (x < h-l) {
        return 1.0 - x/h;
    } else if (x < h+l) {
        return (h + l - x)*(h + l - x)/(4.0 * l * h);
...
double CartesianUGIMP::g(double x, double h, double l){
    if (x < -h-l){
        return 0;
    } else if (x < -h+l){
        return (h + l + x)/(2.0 * l * h);
    } else if (x < -l) {
        return 1.0/h;
    } else if (x < l) {
        return -x/(l * h);
...
void CartesianUGIMP::init(Job* job){
    if (fp64_props.size() < (GRID_DIM+1) || int_props.size() < GRID_DIM){
        std::cout << fp64_props.size() << "\n";
        fprintf(stderr,
                "%s:%s: Need at least %i dimensions and 1 length defined.\n",
...
/*----------------------------------------------------------------------------*/
//hidden initializer
void CartesianUGIMP::hiddenInit(Job* job){//called from loading or initializing functions
...
/*----------------------------------------------------------------------------*/
//
void CartesianUGIMP::writeFrame(Job* job, Serializer* serializer){
...
/*----------------------------------------------------------------------------*/
//
std::string CartesianUGIMP::saveState(Job* job, Serializer* serializer, std::string filepath){
    return "err";
}
...
/*----------------------------------------------------------------------------*/
//
int CartesianUGIMP::loadState(Job* job, Serializer* serializer, std::string fullpath){
    return 0;
}
...
/*----------------------------------------------------------------------------*/
//
int CartesianUGIMP::whichElement(Job* job, KinematicVector& xIN){
    return CartesianLinear::cartesianWhichElement(job, xIN, Lx, hx, Nx, GRID_DIM);
}

/*----------------------------------------------------------------------------*/
//
bool CartesianUGIMP::inDomain(Job* job, KinematicVector& xIN){
...
/*----------------------------------------------------------------------------*/
//
KinematicVector CartesianUGIMP::nodeIDToPosition(Job* job, int idIN){
...
/*----------------------------------------------------------------------------*/
//
void CartesianUGIMP::evaluateBasisFnValue(Job* job, KinematicVector& xIN, std::vector<int>& nID, std::vector<double>& nVAL){
    ...
    for (int n=0;n<nodeIDs.cols();n++){
        ...
        for (int i=0;i<GRID_DIM;i++){
            ...
            if (edge_n(nodeIDs(elementID,n),i) == 2) {
                ...
                tmp *= (s(rst(i), hx(i), l) - s(rst(i)+2*hx(i), hx(i), l));
            } else if (edge_n(nodeIDs(elementID,n),i) == -1) {
                tmp *= (s(rst(i), hx(i), l) - s(rst(i)-2*hx(i), hx(i), l));
            } else if (edge_n(nodeIDs(elementID,n),i) == 0) {
                tmp *= (s(rst(i), hx(i), l) + 2.0*s(std::abs(rst(i))+hx(i), hx(i), l));
            }
        }
...
/*----------------------------------------------------------------------------*/
//
void CartesianUGIMP::evaluateBasisFnGradient(Job* job, KinematicVector& xIN, std::vector<int>& nID, KinematicVectorArray& nGRAD){
    ...
    for (int n=0;n<nodeIDs.cols();n++){
        ...
        for (int i=0;i<GRID_DIM;i++){
            //check proximity to edge
            if (edge_n(nodeIDs(elementID,n),i) == 2) {
                tmpVec(i) = g(rst(i), hx(i), l);
            } else if (edge_n(nodeIDs(elementID,n),i) == 1) {
                tmpVec(i) = (g(rst(i), hx(i), l) - g(rst(i)+2*hx(i), hx(i), l));
            } else if (edge_n(nodeIDs(elementID,n),i) == -1) {
                tmpVec(i) = (g(rst(i), hx(i), l) - g(rst(i)-2*hx(i), hx(i), l));
            } else if (edge_n(nodeIDs(elementID,n),i) == 0) {
                if (rst(i) >= 0) {
                    tmpVec(i) = (g(rst(i), hx(i), l) + 2.0 * g(rst(i) + hx(i), hx(i), l));
                } else if (rst(i) < 0){
                    tmpVec(i) = (g(rst(i), hx(i), l) + 2.0 * g(rst(i) - hx(i), hx(i), l));
                }
            }
        ...
        for (int i=0;i<GRID_DIM;i++){
            ...
            if (edge_n(nodeIDs(elementID,n),i) == 2) {
                for (int j=0; j<GRID_DIM; j++){
                    if (j != i){
                        tmpVec(j) *= s(rst(i), hx(i), l);
                    }
                }
            } else if (edge_n(nodeIDs(elementID,n),i) == 1) {
                for (int j=0; j<GRID_DIM; j++){
                    if (j != i){
                        tmpVec(j) *= (s(rst(i), hx(i), l) - s(rst(i)+2*hx(i), hx(i), l));
                    }
                }
            } else if (edge_n(nodeIDs(elementID,n),i) == -1) {
                for (int j=0; j<GRID_DIM; j++){
                ...
                for (int j=0; j<GRID_DIM; j++){
                    if (j != i){
                        tmpVec(j) *= (s(rst(i), hx(i), l) + 2.0*s(std::abs(rst(i))+hx(i), hx(i), l));
                    }
                }
            }
        }
...
/*----------------------------------------------------------------------------*/
//
double CartesianUGIMP::nodeVolume(Job* job, int idIN){
    return v_n(idIN);
}
...
/*----------------------------------------------------------------------------*/
//
double CartesianUGIMP::elementVolume(Job* job, int idIN){
    return v_e;
}
...
/*----------------------------------------------------------------------------*/
//
int CartesianUGIMP::nodeTag(Job* job, int idIN){
    return -1;
}

/*----------------------------------------------------------------------------*/
//
double CartesianUGIMP::nodeSurfaceArea(Job *job, int idIN) {
    return s_n(idIN);
}

/*----------------------------------------------------------------------------*/
// return edge list
std::vector<int> CartesianUGIMP::getEdgeList(Job* job){
...
/*----------------------------------------------------------------------------*/
//
void CartesianUGIMP::writeHeader(Job *job, Body *body, Serializer *serializer, std::ofstream &nfile, int SPEC) {

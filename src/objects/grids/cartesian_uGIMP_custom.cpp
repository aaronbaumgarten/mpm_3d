...
/*----------------------------------------------------------------------------*/
//
void CartesianUGIMPCustom::init(Job* job){
    if (fp64_props.size() < GRID_DIM+1 || int_props.size() < 2*GRID_DIM){
        std::cout << fp64_props.size() << "\n";
        fprintf(stderr,
                "%s:%s: Need at least %i dimensions, 1 length, and %i properties defined.\n",
...
void CartesianUGIMPCustom::hiddenInit(Job* job){
...
/*----------------------------------------------------------------------------*/
//
std::string CartesianUGIMPCustom::saveState(Job* job, Serializer* serializer, std::string filepath){
    return "err";
}

int CartesianUGIMPCustom::loadState(Job* job, Serializer* serializer, std::string fullpath){
    return 0;
}
...
/*----------------------------------------------------------------------------*/
//
void CartesianUGIMPCustom::fixPosition(Job* job, KinematicVector& xIN){
...
int CartesianUGIMPCustom::whichElement(Job* job, KinematicVector& xIN){
...
bool CartesianUGIMPCustom::inDomain(Job* job, KinematicVector& xIN){
    //adjust xIN
    KinematicVector tmpX = xIN;
    fixPosition(job,tmpX);
...
/*----------------------------------------------------------------------------*/
//
void CartesianUGIMPCustom::evaluateBasisFnValue(Job* job, KinematicVector& xIN, std::vector<int>& nID, std::vector<double>& nVAL){
    KinematicVector tmpX = xIN;
    fixPosition(job,tmpX);
    ...
    for (int n=0;n<nodeIDs.cols();n++){
        ...
        for (int i=0;i<GRID_DIM;i++){
            ...
            if (edge_n(nodeIDs(elementID,n),i) == 2) {
                ...
                tmp *= (s(rst(i), hx(i), l) + s(rst(i) - Lx(i), hx(i), l)); //sum of two shape functions
            } else if (edge_n(nodeIDs(elementID,n),i) == -3) {
                tmp *= (s(rst(i), hx(i), l) + s(rst(i) + Lx(i), hx(i), l)); //sum of two
            }
        }
        //if (tmp < 1e-10){
...
void CartesianUGIMPCustom::evaluateBasisFnGradient(Job* job, KinematicVector& xIN, std::vector<int>& nID, KinematicVectorArray& nGRAD){
    KinematicVector tmpX = xIN;
    fixPosition(job,tmpX);
    ...
    for (int n=0;n<nodeIDs.cols();n++){
        ...
        for (int i=0;i<GRID_DIM;i++){
            ...
            if (edge_n(nodeIDs(elementID,n),i) == 2) {
                ...
                tmpVec(i) = (g(rst(i), hx(i), l) + g(rst(i) - Lx(i), hx(i), l)); //sum of two shape functions
            } else if (edge_n(nodeIDs(elementID,n),i) == -3) {
                tmpVec(i) = (g(rst(i), hx(i), l) + g(rst(i) + Lx(i), hx(i), l)); //sum of two
            }
        }
        ...
        for (int i=0;i<GRID_DIM;i++){
            ...
            if (edge_n(nodeIDs(elementID,n),i) == 2) {
                ...
                for (int j=0; j<GRID_DIM; j++) {
                    if (j != i) {
                        tmpVec(j) *= (s(rst(i), hx(i), l) + s(rst(i) - Lx(i), hx(i), l)); //sum of two shape functions
                    }
                }
            } else if (edge_n(nodeIDs(elementID,n),i) == -3) {
                for (int j=0; j<GRID_DIM; j++) {
                    if (j != i) {
                        tmpVec(j) *= (s(rst(i), hx(i), l) + s(rst(i) + Lx(i), hx(i), l)); //sum of two
                    }
                }
            }
        }

...
/*----------------------------------------------------------------------------*/
//
void CustomDSTModel::init(Job* job, Body* body){
    if (fp64_props.size() < 20){
        std::cout << fp64_props.size() << "\n";
        fprintf(stderr,
                "%s:%s: Need at least 20 properties defined ({E, nu, mu_1, mu_2, a, b, K_3, K_4, K_5, K_6, K_7, K_8, phi_c, phi_j, theta, alpha, grains_rho, eta_0, grains_d, fluid_rho}, {fluid_body}).\n",
                __FILE__, __func__);
        exit(0);
    } else {
        E = fp64_props[0];
        ...
        grains_rho = fp64_props[16];
        eta_0 = fp64_props[17];
        grains_d = fp64_props[18];
        fluid_rho = fp64_props[19];
        ...
        printf("Material properties (E = %g, nu = %g, G = %g, K = %g, mu_1 = %g, mu_2 = %g, a = %g, I_0 = %g K_3 = %g, K_4 = %g, K_5 = %g, K_6 = %g, K_7 = %g, K_8 = %g, phi_c = %g, phi_j = %g, theta =%g, alpha = 0.5, grains_rho = %g, eta_0 = %g, grains_d = %g, fluid_rho = %g).\n",
               E, nu, G, K, mu_1, mu_2, a, I_0, K_3, K_4, K_5, K_6, K_7, K_8, phi_c, phi_j, theta, grains_rho, eta_0, grains_d, fluid_rho);
    }
...
/*----------------------------------------------------------------------------*/
//
void CustomDSTModel::writeFrame(Job* job, Body* body, Serializer* serializer){
...
/*----------------------------------------------------------------------------*/
//
void CustomDSTModel::calculateStress(Job* job, Body* body, int SPEC){
    ...
    for (int i=0;i<body->points->x.size();i++) {
        ...
        if (SPEC == Material::UPDATE) {
            ...
            if (phi(i) > phi_j){
                ...
                c(i) = (c(i) + H*gammap_dot(i)*job->dt)/(1 + H*gammap_dot(i)*job->dt + job->dt/(S*t_k));
            }
        }

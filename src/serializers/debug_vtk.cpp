...
void serializerInit(Job* job){
    if (job->serializer.str_props.size() < 3 || job->serializer.fp64_props.size() < 2){
        std::cout << job->serializer.str_props.size() << "\n";
        fprintf(stderr,
                "%s:%s: Need at least 5 properties defined ({sampleRate, t_debug}, {frameDirectory, outputDirectory, outputName}).\n",
                __FILE__, __func__);
        exit(0);
    } else {
        ...
        t_debug = job->serializer.fp64_props[1];
        sampledFrames = 0;
        t_last_frame = 0;

        printf("Serializer properties (frameDirectory = %s, outputDirectory = %s, outputName = %s, sampleRate = %g, t_debug = %g).\n",
               frameDirectory.c_str(), outputDirectory.c_str(), outputName.c_str(), sampleRate, t_debug);
    }
...
int serializerWriteFrame(Job* job){
    if (job->t >= t_debug || (job->t - t_last_frame) >= (1.0/sampleRate) || sampledFrames == 0){ //job->t >= sampledFrames/sampleRate){

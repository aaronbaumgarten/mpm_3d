...
//explicit update stress last with enhanced strain
...
void applyVolumeSmoothing(Job* job){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        ...
        pvec.resize(job->bodies[b].points.x.rows());

        //strain smoothing
        pvec = (job->bodies[b].points.v.array()/job->bodies[b].points.v0.array()).array().log();
        pvec = pvec.array()*job->bodies[b].points.m.array();
        alpha.resize(job->bodies[b].nodes.x.rows());
        job->bodies[b].bodyCalcNodalValues(job,alpha,pvec,Body::SET); //cast strain to nodes
        alpha = alpha.array() / job->bodies[b].nodes.m.array();
        job->bodies[b].bodyCalcPointValues(job,pvec,alpha,Body::SET); //cast back to points
        job->bodies[b].points.v = job->bodies[b].points.v0.array() * pvec.array().exp();

        //pressure smoothing
        for (size_t i=0;i<pvec.rows();i++){
            if (job->DIM == 1){
                pvec(i) = -job->bodies[b].points.T(i,0);
            } else if (job->DIM == 2) {
                pvec(i) = -(job->bodies[b].points.T(i,0)+job->bodies[b].points.T(i,3))/2.0;
            } else if (job->DIM == 3) {
                pvec(i) = -(job->bodies[b].points.T(i,job->XX) +
                            job->bodies[b].points.T(i,job->YY) +
                            job->bodies[b].points.T(i,job->ZZ))/3.0;
            }
        }
        pvec = pvec.array() * job->bodies[b].points.m.array();
        beta.resize(job->bodies[b].nodes.x.rows());
        job->bodies[b].bodyCalcNodalValues(job,beta,pvec,Body::SET); //cast pressure to nodes
        beta = beta.array() / job->bodies[b].nodes.m.array();
        job->bodies[b].bodyCalcPointValues(job,pvec,beta,Body::SET); //cast pressure to points
        for (size_t i=0;i<pvec.rows();i++){
            job->bodies[b].material.materialAssignPressure(job,&(job->bodies[b]),pvec(i),i,Material::UPDATE);
        }
    }
    return;
}

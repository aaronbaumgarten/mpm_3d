...
void solverInit(Job* job){
    if (job->solver.fp64_props.size() < 2 && job->solver.int_props.size() > 1){
        ...
        printf("Solver properties (TOL = %g, linear_step = %g).\n",
               TOL, h);
    }
...
void sizeVectors(Job* job){
    mv_k.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    mv_n.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    f_n.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    m.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    s.resize(job->bodies.size() * job->DIM * job->grid.node_count);

    Fv.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    Fv_plus.resize(job->bodies.size() * job->DIM * job->grid.node_count);

    p.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    r.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    Ap.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    tmpVec.resize(job->bodies.size() * job->DIM * job->grid.node_count);

    s_min.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    mv_n_trial.resize(job->bodies.size() * job->DIM * job->grid.node_count);
    return;
}
...
void fillMomentumVector(Job* job, Eigen::VectorXd& mvOut){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        ...
        if(job->activeBodies[b] == 0){
            mvOut.block(body_start_pos, 1, job->DIM * job->grid.node_count, 1).setZero();
        } else {
            for (size_t pos = 0; pos < job->DIM; pos++) {
                rel_start_pos = pos * job->grid.node_count;
                mvOut.block(body_start_pos + rel_start_pos, 1, job->grid.node_count, 1) = job->bodies[b].nodes.mx_t.col(
                        pos);
            }
        }
    }
...
void fillForceVector(Job* job, Eigen::VectorXd& fOut){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        body_start_pos = b * job->DIM * job->grid.node_count;
        if(job->activeBodies[b] == 0){
            fOut.block(body_start_pos, 1, job->DIM * job->grid.node_count, 1).setZero();
        } else {
            for (size_t pos = 0; pos < job->DIM; pos++) {
                rel_start_pos = pos * job->grid.node_count;
                fOut.block(body_start_pos + rel_start_pos, 1, job->grid.node_count, 1) = job->bodies[b].nodes.f.col(
                        pos);
            }
        }
    }
...
void fillMassVector(Job* job, Eigen::VectorXd& mOut){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        body_start_pos = b * job->DIM * job->grid.node_count;
        if(job->activeBodies[b] == 0){
            mOut.block(body_start_pos, 1, job->DIM * job->grid.node_count, 1).setZero();
        } else {
            for (size_t pos = 0; pos < job->DIM; pos++) {
                rel_start_pos = pos * job->grid.node_count;
                mOut.block(body_start_pos + rel_start_pos, 1, job->grid.node_count, 1) = job->bodies[b].nodes.m;
            }
        }
    }
...
void pushMomentumVector(Job* job, Eigen::VectorXd& mvIn){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        ...
        if(job->activeBodies[b] == 0){
            ...
            for (size_t pos = 0; pos < job->DIM; pos++) {
                rel_start_pos = pos * job->grid.node_count;
                job->bodies[b].nodes.mx_t.col(pos) = mvIn.block(body_start_pos + rel_start_pos, 1, job->grid.node_count, 1);
...
void pushForceVector(Job* job, Eigen::VectorXd& fIn){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        ...
        if(job->activeBodies[b] == 0){
            ...
            for (size_t pos = 0; pos < job->DIM; pos++) {
                rel_start_pos = pos * job->grid.node_count;
                job->bodies[b].nodes.f.col(pos) = fIn.block(body_start_pos + rel_start_pos, 1, job->grid.node_count, 1);
            }
        }
    }
}
...
void solverStep(Job* job){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        ...
        tmp_bodies[b].points = Points(job->bodies[b].points);
        tmp_bodies[b].nodes = Nodes(job->bodies[b].nodes);
    }
    ...
    while (mv_n_res / mv_k.norm() > TOL && n < maxIter) {
        ...
        while (rnorm / bnorm > TOL && k < maxIter) {
            ...
            if (mv_n.norm() > 0) {
                tmpVec = s + h * p * s.norm() / p.norm(); //velocity
            } else {
                tmpVec = h * p / p.norm();
            }
            ...
            //approximate J*s ---------------------------------
            if (mv_n.norm() > 0) {
                Ap = p.norm() / (h * mv_n.norm()) * (Fv_plus - Fv); //momentum
            } else {
...
std::string solverSaveState(Job* job, Serializer* serializer, std::string filepath){
    ...
    for (size_t b=0;b<job->bodies.size();b++){
        ...
        job->bodies[b].points = Points(tmp_bodies[b].points);
        job->bodies[b].nodes = Nodes(tmp_bodies[b].nodes);
    }
